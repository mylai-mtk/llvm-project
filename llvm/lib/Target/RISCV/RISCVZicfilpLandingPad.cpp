...
#define DEBUG_TYPE "riscv-zicfilp-landing-pad"
#define PASS_NAME "Insert RISCV Zicfilp callee landing pad"
...
INITIALIZE_PASS(RISCVZicfilpLandingPad, "riscv-zicfilp-landing-pad",
...
bool RISCVZicfilpLandingPad::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (!M->getModuleFlag("zicfilp-cfi"))
    return false;
...
bool RISCVZicfilpLandingPad::addLpad(MachineBasicBlock &MBB, const uint32_t Label) {
  ...
  LLVM_DEBUG(dbgs() << "Adding lpad " << Label << " to " << MBB.getName()
                    << "\n");
