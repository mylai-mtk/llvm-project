...
void RISCVCfiLpSimplePLT::writePltHeader(uint8_t *buf) const {
  ...
  // nop
  // nop
  uint32_t offset = in.gotPlt->getVA() - (in.plt->getVA() + 4 /* offset for lpad */);
  uint32_t load = config->is64 ? LD : LW;
...
void RISCVCfiLpSimplePLT::writePlt(uint8_t *buf, const Symbol &sym,
                                   uint64_t pltEntryAddr) const {
  // 1: lpad 0
  // auipc t3, %pcrel_hi(f@.got.plt)
  // l[wd] t3, %pcrel_lo(1b)(t3)
  // jalr t1, t3
  uint32_t offset = sym.getGotPltVA() - (pltEntryAddr + 4 /* offset for lpad */);
  write32le(buf + 0, utype(AUIPC, 0, 0)); // lpad 0
...
TargetInfo *elf::getRISCVTargetInfo() {
  if ((config->andFeatures & GNU_PROPERTY_RISCV_FEATURE_1_ZICFILP) ||
      config->zForceZicfilp) {
