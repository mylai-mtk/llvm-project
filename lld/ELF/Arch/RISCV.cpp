...
public:
  ...
  virtual uint32_t getLpadVal(const Symbol &sym) const;
};
...
uint32_t
RISCV::getLpadVal(const Symbol &sym) const {
  Err(ctx) << "failed to get RISCV lpad value for symbol: " << sym.getName();
  return 0;
}
...
public:
  ...
  uint32_t getLpadVal(const Symbol &sym) const override;
...
uint32_t
RISCVCfiLpLabeledPLT::getLpadVal(const Symbol &sym) const {
...
uint32_t elf::getRISCVLpadValue(Ctx &ctx, const Symbol &sym) {
  return static_cast<RISCV *>(ctx.target.get())->getLpadVal(sym);
}
