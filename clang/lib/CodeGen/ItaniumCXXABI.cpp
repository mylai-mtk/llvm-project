...
CGCallee ItaniumCXXABI::EmitLoadOfMemberFunctionPointer(
...

  {
    ...
    if (CGM.UseRISCVZicfilpFuncSigCFI) {
      ...
      CGF.EmitRISCVSetLpadLabelIntrin(Label, FuncSig);
    }
...
ItaniumCXXABI::getOrCreateVirtualFunctionPointerThunk(const CXXMethodDecl *MD) {
  ...
  const CGFunctionInfo &CallInfo = CGM.getTypes().arrangeCXXMethodCall(
      CallArgs, FPT, Required,
      /*numPrefixArgs=*/0, /*isMemberPointerCall=*/false);
