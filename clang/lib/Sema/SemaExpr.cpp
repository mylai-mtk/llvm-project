...
ExprResult Sema::BuildResolvedCallExpr(Expr *Fn, NamedDecl *NDecl,
  ...
  if (Proto) {
    ...
    if (!Proto && !Args.empty()) {
      ...
      // -Wdeprecated-non-prototype was disabled where the function was
      // declared. If so, we will silence the diagnostic here on the assumption
      // that this interface is intentional and the user knows what they're
      // doing. We will also silence the diagnostic if there is a function
      // declaration but it was implicitly defined (the user already gets
      // diagnostics about the creation of the implicit function declaration,
      // so the additional warning is not helpful).
      if (!FDecl || (!FDecl->isImplicit() &&
                     !Diags.isIgnored(diag::warn_strict_uses_without_prototype,
                                      FDecl->getLocation())))
        Diag(LParenLoc, diag::warn_strict_uses_without_prototype)
            << (FDecl != nullptr) << FDecl;

      if (!FDecl && getLangOpts().CFProtectionBranch &&
          getLangOpts().getCFBranchLabelScheme() ==
              CFBranchLabelSchemeKind::FuncSig)
        Diag(LParenLoc,
             diag::warn_indirect_call_noproto_with_args_breaks_prototype_cfi);
    }
